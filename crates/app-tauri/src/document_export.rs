...
use docx_rs::{
    ...
    Numbering, NumberingId, Paragraph, Run, RunFonts, Start, Style, StyleType,
};
...
///
/// MVP-Scope gemäß Spec: Überschriften (`#`–`###`), Absätze, Fett/Kursiv,
/// Aufzählungen/nummerierte Listen. Tabellen werden bewusst **nicht** über
/// `Options::ENABLE_TABLES` aktiviert — dadurch behandelt `pulldown-cmark`
/// eine Markdown-Tabelle automatisch als reinen Absatztext (z. B. Zeilen mit
/// `|`-Zeichen), was schon exakt die von der Spec verlangte vereinfachte
/// Darstellung ergibt, ohne einen eigenen Tabellen-Renderer zu brauchen.
/// Code-Blöcke werden als eigener Absatz in Monospace-Schrift ohne
/// Syntax-Highlighting ausgegeben (Spec 0012, Abschnitt 4, wörtliches
/// Beispiel). Verschachtelte Listen bekommen bis zu [`LIST_INDENT_LEVELS`]
/// unterscheidbare Einrückungen, tiefere Ebenen flachen auf die letzte ab.
...
fn heading_style_id(level: HeadingLevel) -> &'static str {
    match level {
        ...
        // Spec-Scope deckt nur h1-h3 ab; tiefere Überschriften vereinfacht
        // auf den Heading3-Stil abgebildet statt sie unformatiert zu lassen.
        HeadingLevel::H3 | HeadingLevel::H4 | HeadingLevel::H5 | HeadingLevel::H6 => "Heading3",
    }
}

/// Eine Aufzählungs-/Nummerierungsdefinition mit [`LIST_INDENT_LEVELS`]
/// Ebenen, alle mit demselben Format/Zeichen, aber wachsender Einrückung
/// (720 Twips ≈ 0,5in je Ebene) — reicht für die von der Spec verlangte
/// vereinfachte Verschachtelungsdarstellung.
...
/// [`Paragraph`]s auf. Bewusst als eigene Funktion (statt Teil von
/// [`markdown_to_docx_bytes`]): reine Datenumwandlung ohne den
/// Docx-Grundgerüst-Aufbau, dadurch leichter im Kopf zu behalten.
    ...
    for event in Parser::new_ext(content_markdown, Options::empty()) {
        match event {
            Event::Start(Tag::Heading { level, .. }) => {
                ...
                current = Some(Paragraph::new().style(heading_style_id(level)));
            }
            ...
            // Absätze innerhalb eines "tight" List-Items kommen ohne eigenes
            // Paragraph-Start/End (CommonMark-Verhalten) — der schon von
            // `Start(Item)` gesetzte, nummerierte `current` bleibt dann
            // unverändert erhalten, statt überschrieben zu werden.
            ...
            // Alles außerhalb des MVP-Scopes (Bilder, Links als Hyperlink,
            // Blockzitate als eigener Stil, HTML, Fußnoten, ...) wird bewusst
            // vereinfacht: Start/End-Marker werden ignoriert, ihr Textinhalt
            // läuft trotzdem über die obigen `Event::Text`-Fälle mit ein.
            _ => {}
        }
    }
    flush(&mut current, &mut paragraphs);

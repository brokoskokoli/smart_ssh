...
pub(crate) fn fallback_system_prompt_addition(actions: &[ActionSchema]) -> String {
    ...
    addition.push_str(
        " aus, im Format {\"action\": \"<name>\", \"parameters\": {...}}. \
         ...
         Gib niemals mehr als einen solchen Block aus. Verfügbare Aktionen und deren Parameter:\n",
    );
    for action in actions {
        let params_summary = action
            .parameters
            .iter()
        ...
        addition.push_str(&format!(
            ...
            action.name, action.description, action.name, params_summary
        ));
    }
    addition
}

...
data: [DONE]\n\n";
    ...
    let provider = OpenAiCompatibleProvider::new(server.uri(), "gpt-test", "test-key", true);
...
data: [DONE]\n\n";
    let server = mock_server_with_sse_body(sse_body).await;
    let provider = OpenAiCompatibleProvider::new(server.uri(), "gpt-test", "test-key", false);
...
async fn test_authentication_failure_maps_401_to_ai_error() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let provider = OpenAiCompatibleProvider::new(server.uri(), "gpt-test", "bad-key", true);
